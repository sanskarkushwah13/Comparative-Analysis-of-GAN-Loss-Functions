...
def get_dataloader(batch_size=64):
    ...
    print(f"  CIFAR-10 Total: {len(dataset)} images")
...
class Discriminator(nn.Module):
    ...
    def __init__(self, ndf=64, use_sigmoid=False):
        ...
        if use_sigmoid:
            layers.append(nn.Sigmoid())  # ✅ Only Standard GAN
...
def train(loss_type, out_dir, epochs=30, inception=None, fid_every=10):
    ...
    print(f"\n{'='*60}")
    print(f"  DCGAN + CIFAR-10 | {loss_type.upper()} | {epochs} Epochs")
...
def plot_all(all_g, all_d, all_fid, out_dir):
    ...
    plt.suptitle("DCGAN CIFAR-10 — All 5 Loss Functions (30 Epochs)", fontsize=13)
    plt.tight_layout()
    path = os.path.join(out_dir, "cifar_loss_curves.png")
    ...
    plt.legend()
    plt.grid(True)
    path = os.path.join(out_dir, "cifar_fid_scores.png")
    plt.savefig(path, dpi=300)
    plt.close()
...
def run_experiments(epochs=30, fid_every=10):

    # Output folder
    out_dir = os.path.join(os.path.expanduser("~"),
                           "project", "cifar_output")
    ...
    for i, loss_type in enumerate(experiments):
        ...
        pd.DataFrame(results).to_csv(
            os.path.join(out_dir, "cifar_results_partial.csv"), index=False)
        print(f"  📊 Partial CSV saved")

    # Final CSV
    df       = pd.DataFrame(results)
    csv_path = os.path.join(out_dir, "cifar_results.csv")
    ...
    print(f"\n{'#'*60}")
    print("  🎉 ALL 5 CIFAR-10 EXPERIMENTS COMPLETE!")
    print(f"  📁 Output: {out_dir}")
    print(f"{'#'*60}")
    print(f"""
  Files:
...
if __name__ == "__main__":
    run_experiments(epochs=100, fid_every=10)
